...
`NapCatError` 使用 `INVALID_CONFIG` 和 `ABORTED`。超时和调用方取消统一为 `ABORTED`，仅超时消息包含毫秒数。SDK 的连接或发送异常保持原错误抛出，由 core 包装为 `SEND_FAILED`；连接和 target 配置错误由 core 包装为带对应路径的 `INVALID_CONFIG`。

Vitest 通过注入 mock client 覆盖 target 默认字段、partial 继承、连接字段拒绝、同一账号的私聊/群聊参数映射、连接参数、receipt、失败清理、配置校验和超时取消。测试不得依赖真实 NapCat 服务。tsdown 以 Node 平台输出 ESM 与类型声明。
